-- V10: Batched pair stats to avoid quadratic disk usage
--
-- Replaces the single global self-join in refresh_format_stats with a loop
-- that processes p_batch_size decks at a time, accumulating counts into a
-- real staging table. No accuracy loss: batches are non-overlapping and cover
-- every deck, so summed counts are identical to the global result.
--
-- _card_pair_staging is a real table (not ON COMMIT DROP) so a killed run
-- leaves it behind — the DROP IF EXISTS at the top of the procedure cleans it
-- up before the next run begins.
...
BEGIN
    ...
    -- ── 1. Card stats — linear scan, no quadratic growth ─────────────────────
    ...
    -- ── 2. Pair accumulation — p_batch_size decks at a time ──────────────────
    ...
    LOOP
        ...
        -- Deduplicate (deck_id, card_id) within the batch before self-joining
        -- so a card appearing in multiple boards counts as one per deck.
