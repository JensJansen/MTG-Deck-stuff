-- V29: Remove board filtering from refresh_format_stats
--
-- Decks are now treated as atomic units — every card_id in {format}_deck_cards
-- is included regardless of which board it appears on. This eliminates the
-- p_boards parameter and all board = ANY(...) filter clauses.
--
-- The (deck_id, card_id) index from V24 now covers the hot batch_dc query
-- perfectly since board is no longer a filter condition.
