-- V30: Use UNLOGGED table with fillfactor=50 for pair accumulation staging
--
-- The staging table created in V29 was a regular (logged) heap. Under MVCC,
-- every ON CONFLICT DO UPDATE writes a dead tuple, causing progressive index
-- bloat as batches accumulate. WAL writes add further overhead for what is
-- purely ephemeral computation state.
--
-- UNLOGGED skips WAL entirely. fillfactor=50 reserves half of each page for
-- in-place HOT updates, keeping the (card_id_a, card_id_b) index clean
-- throughout the run. Safe because the table is always dropped at the end of
-- the procedure and rebuilt from scratch on the next call.
