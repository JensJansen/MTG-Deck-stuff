-- V34: Stats procedure for singleton formats (commander, canadian_highlander)
--
-- Singleton decks store cards as a JSONB array on the deck row rather than
-- in a separate deck_cards table. This procedure mirrors refresh_format_stats
-- but extracts (deck_id, card_id) pairs by unnesting that JSONB column.
--
-- avg_quantity is always 1.0 in practice (singleton rule) but is kept for
-- schema consistency with the regular-format stats tables.
