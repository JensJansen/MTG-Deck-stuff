...
def compute_card_presence(
    ...
    """
    Returns:
        presence_matrix : scipy csr_matrix (N, V) bool
    ...
    matrix = sp.csr_matrix((data, (rows, cols)), shape=(N, V))

    card_vocab = [None] * V
    for name, idx in card_index.items():
        card_vocab[idx] = name

    return matrix, card_vocab
...
def save_presence(fmt: str, deck_ids: list[str], matrix: sp.csr_matrix, card_vocab: list[str]) -> None:
    np.savez_compressed(
